...
export default function AdminAnalyticsPage() {
  ...
  if (!data) {
    return <div className="text-slate-400 font-bold">Failed to load analytics.</div>;
  }

  const statCards = [
    { label: "Total Teachers", value: data.overview.totalTeachers, icon: <GraduationCap size={20} />, color: "bg-blue-600" },
    { label: "Total Students", value: data.overview.totalStudents, icon: <Users size={20} />, color: "bg-emerald-600" },
    { label: "Total Courses", value: data.overview.totalCourses, icon: <BookOpen size={20} />, color: "bg-amber-500" },
    { label: "Completion Rate", value: `${data.overview.completionRate}%`, icon: <TrendingUp size={20} />, color: "bg-slate-700" },
  ];

  return (
    <div className="space-y-10 animate-in fade-in duration-700 text-left">
      <div>
        <h2 className="text-3xl font-black text-slate-800 mb-1">Analytics</h2>
        <p className="text-slate-400 font-medium">Overview of your organization's activity</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <h3 className="text-xl font-black text-slate-800 mb-6">Enrollments (Last 6 Months)</h3>
          {data.enrollmentTrend?.length > 0 ? (
            ...
            <p className="text-slate-400 text-center py-10 font-bold italic">No enrollment data yet.</p>
          )}
        </div>

        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <h3 className="text-xl font-black text-slate-800 mb-6">Student Progress Distribution</h3>
              ...
              <div className="space-y-2 flex-1">
                {data.studentProgressDistribution.map((d: any, i: number) => (
                  <div key={d.range} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-400 text-xs ml-auto">{d.count} students</span>
                  </div>
                ))}
              </div>
            </div>
          ) : (
            <p className="text-slate-400 text-center py-10 font-bold italic">No progress data yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <h3 className="text-xl font-black text-slate-800 mb-6">Top Courses</h3>
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <h3 className="text-xl font-black text-slate-800 mb-6">Teacher Leaderboard</h3>
          {data.teacherLeaderboard?.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.teacherLeaderboard.map((t: any) => (
                    <tr key={t.teacherId} className="border-b border-slate-50 last:border-0">
                      <td className="py-3 font-bold text-slate-700">{t.name}</td>
                      <td className="py-3 text-slate-500">{t.studentCount}</td>
                      <td className="py-3 text-slate-500">{t.completionRate}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <p className="text-slate-400 text-center py-10 font-bold italic">No teachers yet.</p>
          )}
        </div>
      </div>

      <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
        <h3 className="text-xl font-black text-slate-800 mb-6">Quiz Pass Rates</h3>
        {data.quizAnalytics?.length > 0 ? (
          ...
          <p className="text-slate-400 text-center py-10 font-bold italic">No quiz data yet.</p>
        )}
      </div>
    </div>
  );
