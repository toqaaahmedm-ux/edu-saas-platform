...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: t("totalRevenue"), value: `EGP ${Number(totalRevenue).toLocaleString()}`, icon: <CreditCard />, color: "bg-emerald-600" },
    { label: t("totalStudents"), value: usersLoading ? "..." : totalStudents, icon: <Users />, color: "bg-blue-600" },
    { label: t("totalCourses"), value: coursesLoading ? "..." : courses.length, icon: <LayoutGrid />, color: "bg-amber-500" },
    { label: t("systemHealth"), value: "100%", icon: <ShieldCheck />, color: "bg-slate-700" },
  ];
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast.success(t("courseApprovedToast"));
    } catch {
  ...
  const handleReject = async (id: string, title: string) => {
    try {
      ...
      deleteCourse(id, {
        ...
        onError: () => toast.success(t("courseArchivedToast", { title })),
      });
    } catch {
  ...
  const handleDeleteUser = (id: string) => {
    deleteUser(id, {
      onSuccess: () => toast.success(t("userDeletedToast")),
      onError: () => toast.error(t("deleteUserFailedToast")),
    });
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-700 text-left">
      <div className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div className="relative z-10">
          <h2 className="text-3xl font-black mb-2">{t("centralAdminPanel")} 🔐</h2>
          <p className="text-slate-400 font-medium text-lg italic">
            {t("welcomeBack", { name: user?.name || t("defaultAdminName"), count: users.length })}
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <div className="flex justify-between items-center mb-8 border-b pb-4">
            <h3 className="text-xl font-black text-slate-800">{t("courseManagement")}</h3>
            <span className="text-sm bg-blue-100 text-blue-600 px-4 py-1 rounded-full font-bold">
              {t("coursesCountLabel", { count: courses.length })}
            </span>
          </div>
          {coursesLoading ? (
                ...
                <div key={course.id} className="flex flex-col md:flex-row justify-between items-center p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{t("instructorLabel")}: <span className="text-blue-600 font-bold">{typeof course.instructor === 'string' ? course.instructor : course.instructor?.name || t("unknownInstructor")}</span></p>
                    </div>
                  </div>
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          <h3 className="text-xl font-black text-slate-800 mb-6">{t("recentUsers")}</h3>
          {usersLoading ? (
            <div className="flex justify-center py-10">
              <Loader2 className="animate-spin text-blue-600" size={32} />
            </div>
            ...
            <div className="space-y-4">
              {users.slice(0, 5).map((u: any) => (
                <div key={u.id} className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-xl transition-all">
                  ...
                  <button onClick={() => handleDeleteUser(u.id)} className="text-slate-300 hover:text-red-500 transition-colors" aria-label={t("deleteUserAria")}>
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
