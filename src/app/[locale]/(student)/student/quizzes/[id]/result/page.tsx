"use client";

import { useParams } from "next/navigation";
import { useLatestAttempt, useMyAttempts } from "@/hooks/useQuizzes";
import { useAuthStore } from "@/store/useAuthStore";
import { Certificate } from "@/components/student/Certificate";
import Link from "next/link";
import { Award, RefreshCcw, Home, Download, History, Loader2, CheckCircle2, XCircle } from "lucide-react";
import { useState } from "react";

export default function ResultPage() {
  const { id: quizId } = useParams<{ id: string }>();
  const user = useAuthStore((s) => s.user);
  const [showReview, setShowReview] = useState(false);

  const { data: attempt, isLoading, isError } = useLatestAttempt(quizId);
  const { data: attempts } = useMyAttempts(quizId);

  if (isLoading) {
    return (
      <div className="flex justify-center items-center p-20">
        <Loader2 className="animate-spin text-blue-600" size={40} />
      </div>
    );
  }

  if (isError || !attempt) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <p className="text-red-400 font-bold">No result found for this quiz</p>
        <Link href="/student/quizzes" className="px-6 py-3 bg-slate-100 rounded-xl text-slate-600 font-bold">
          Back to Quizzes
        </Link>
      </div>
    );
  }

  const { score, passed, bestScore, attemptsUsed, attemptsRemaining, certificate, answers } = attempt;

  return (
    <div className="w-full flex flex-col items-center py-10 px-4 text-left page-transition">
      <div className="max-w-5xl w-full space-y-6">
        <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-2xl border border-blue-50 text-center print:shadow-none print:border-none">
          <div className={`w-24 h-24 rounded-full mx-auto flex items-center justify-center mb-6 print:hidden ${passed ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
            <Award size={48} />
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-800 mb-2 print:hidden">
            {passed ? "Congratulations!" : "Hard Luck! Keep trying"}
          </h1>
          <p className="text-gray-500 text-xl mb-8 font-medium print:hidden">
            Your final score is{" "}
            <span className="text-blue-600 font-black text-4xl">{score}%</span>
          </p>

          <div className="grid grid-cols-3 gap-4 mb-8 print:hidden">
            <div className="bg-blue-50 p-4 rounded-2xl">
              <p className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-1">Current</p>
              <p className="text-3xl font-black text-blue-600">{score}%</p>
            </div>
            <div className="bg-emerald-50 p-4 rounded-2xl">
              <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-1">Best</p>
              <p className="text-3xl font-black text-emerald-600">{bestScore}%</p>
            </div>
            <div className="bg-purple-50 p-4 rounded-2xl">
              <p className="text-xs font-bold text-purple-400 uppercase tracking-widest mb-1">Attempts</p>
              <p className="text-3xl font-black text-purple-600">{attemptsUsed}/3</p>
            </div>
          </div>

          {passed ? (
            <div className="mb-10 space-y-6 animate-in fade-in slide-in-from-bottom-5">
              {certificate ? (
                <p className="text-emerald-600 font-bold text-sm">Certificate saved to your account</p>
              ) : (
                <p className="text-amber-500 font-bold text-sm">Certificate not issued yet - make sure the full course is completed.</p>
              )}
              {certificate && (
                <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed border-slate-200 print:bg-white print:border-none">
                  <h3 className="text-sm font-bold text-slate-400 mb-6 uppercase tracking-widest print:hidden">
                    Official Certificate Preview
                  </h3>
                  <div className="flex justify-center overflow-hidden h-[300px] md:h-[450px] print:h-auto">
                    <div className="scale-[0.45] md:scale-[0.65] lg:scale-[0.8] origin-top print:scale-100">
                      <Certificate name={user?.name || "Student Name"} score={score} />
                    </div>
                  </div>
                </div>
              )}
              {certificate && (
                <button
                  onClick={() => window.print()}
                  className="flex items-center gap-2 mx-auto bg-blue-600 text-white px-10 py-4 rounded-2xl font-black hover:bg-blue-700 transition-all shadow-xl active:scale-95 print:hidden"
                >
                  <Download size={20} /> Save Certificate as PDF
                </button>
              )}
            </div>
          ) : (
            <div className="mb-10 p-6 bg-red-50 rounded-2xl border border-red-100 text-red-600 font-bold print:hidden">
              You need at least 70% to earn a certificate. Don't give up!
            </div>
          )}

          {answers && answers.length > 0 && (
            <div className="mb-10 print:hidden">
              <button
                onClick={() => setShowReview((v) => !v)}
                className="text-blue-600 font-bold text-sm underline"
              >
                {showReview ? "Hide" : "Show"} Answer Review
              </button>
              {showReview && (
                <div className="mt-6 space-y-4 text-left">
                  {answers.map((a: any, i: number) => (
                    <div key={a.questionId} className={`p-5 rounded-2xl border-2 ${a.isCorrect ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}>
                      <div className="flex items-start gap-3">
                        {a.isCorrect ? <CheckCircle2 className="text-emerald-600 shrink-0 mt-1" size={20} /> : <XCircle className="text-red-500 shrink-0 mt-1" size={20} />}
                        <div className="flex-1">
                          <p className="font-bold text-slate-800 mb-2">{i + 1}. {a.questionText}</p>
                          <div className="grid grid-cols-1 gap-1 text-sm">
                            {a.options.map((opt: string, oi: number) => (
                              <div
                                key={oi}
                                className={
                                  oi === a.correctIndex
                                    ? "px-3 py-1 rounded-lg bg-emerald-100 text-emerald-700 font-bold"
                                    : oi === a.selectedAnswer
                                      ? "px-3 py-1 rounded-lg bg-red-100 text-red-600 font-bold"
                                      : "px-3 py-1 rounded-lg text-slate-500"
                                }
                              >
                                {opt}
                              </div>
                            ))}
                          </div>
                        </div>
                      </div>
                    </div>
                  ))}
                </div>
              )}
            </div>
          )}

          <div className="flex flex-wrap justify-center gap-4 border-t border-slate-100 pt-10 print:hidden">
            {attemptsRemaining > 0 && (
              <Link
                href={`/student/quizzes/${quizId}`}
                className="flex items-center gap-2 px-8 py-4 border-2 border-blue-100 text-blue-600 rounded-2xl font-bold hover:bg-blue-50 transition-all"
              >
                <RefreshCcw size={20} /> Retake Quiz
              </Link>
            )}
            <Link
              href="/student/dashboard"
              className="flex items-center gap-2 px-8 py-4 bg-slate-100 text-slate-700 rounded-2xl font-bold hover:bg-slate-200 transition-all"
            >
              <Home size={20} /> Back to Dashboard
            </Link>
          </div>
        </div>

        {attempts && attempts.length > 1 && (
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 print:hidden">
            <h2 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-2">
              <History size={20} /> Result History
            </h2>
            <div className="space-y-3">
              {attempts.map((r: any) => (
                <div key={r.attemptId} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50">
                  <div className="flex items-center gap-3">
                    <span className={`w-3 h-3 rounded-full ${r.passed ? "bg-emerald-500" : "bg-red-400"}`} />
                    <span className="text-sm font-bold text-slate-600">
                      {new Date(r.submittedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                  <span className={`font-black text-lg ${r.passed ? "text-emerald-600" : "text-red-500"}`}>
                    {r.score}%
                  </span>
                </div>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
