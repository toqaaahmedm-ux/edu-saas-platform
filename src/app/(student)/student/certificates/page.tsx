...
export default function CertificatesPage() {
  // [تقرير 1 - صفحة 1]: شيلت calculateScore عشان الحساب بقا سيرفر (Fix BUG-04)
  // بنقرأ الـ score اللي السيرفر بعته ومتخزن في الـ store علطول
  ...
  // الطالب بياخد الشهادة لو درجته 50 أو أكتر (Logic Fix)
  const hasCertificate = score !== null && score >= 50;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 text-left pb-10">
      ...
      {/* هيدر المركز.. بستخدم بيانات اليوزر من الـ Store (Fix BUG-07) */}
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          ...
          <span className={`text-white text-[10px] px-4 py-1.5 rounded-full font-black uppercase tracking-tighter ${hasCertificate ? "bg-emerald-500" : "bg-slate-400"}`}>
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 font-bold uppercase tracking-widest mt-1">
                      Issuer: <span className="text-blue-600">EduSaaS Platform</span>
                    </p>
                  </div>
                </div>
              ...
              {showPreview && (
                <div className="p-12 bg-slate-900 flex justify-center animate-in zoom-in duration-500 relative overflow-hidden">
                  ...
                  <div className="scale-[0.45] md:scale-[0.75] lg:scale-[1] origin-top shadow-2xl">
                    {/* بنمرر الدرجة الحقيقية للشهادة */}
                    <Certificate name={user?.name || "Student Name"} score={score || 0} />
                  </div>
                </div>
              )}
            </div>
