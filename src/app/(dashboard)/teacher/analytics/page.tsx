...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // بعد — كله بيكلم NestJS مباشرة
        const [statsRes, analyticsRes] = await Promise.all([
          apiClient.get('/courses/teacher/stats'),
          apiClient.get('/courses/teacher/analytics'),
        ]);
    ...
    <div className="space-y-10 animate-in fade-in duration-700 text-left">
        ...
        {statCards.map((stat) => (
          <div key={stat.label} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="flex items-center gap-1 text-sm font-black text-emerald-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          {performanceTrend.length === 0 || (performanceTrend.length === 1 && performanceTrend[0].month === 'No data') ? (
              ...
              {performanceTrend.map((bar: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex-1 group relative flex flex-col items-center">
                    <div
                      className="w-full bg-blue-100 group-hover:bg-blue-600 rounded-t-xl transition-all duration-700 cursor-pointer"
                      style={{ height: `${heightPct}%` }}
                    >
                      <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity font-bold whitespace-nowrap">
                        {bar.score}%
                      </div>
                    </div>
        ...
        <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-6 relative z-10">
              ...
              return (
                <div key={item.name} className="space-y-2">
                  ...
                  <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                    <div
                      className={`${colors[index]} h-full transition-all duration-1000 ease-out`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
            ...
            {quizDistribution.every((i: any) => i.value === 0) && (
              <p className="text-slate-400 text-center py-4">No quiz attempts yet</p>
            )}
          </div>
