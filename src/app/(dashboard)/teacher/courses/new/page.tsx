...
export default function NewCoursePage() {
  ...
  const onSubmit = async (data: CourseInput) => {
    ...
    try {
      // إرسال البيانات للـ API Layer (SaaS Standard)
      await coursesApi.create(data);

      // تحديث المتجر المحلي باسم المدرس الحقيقي (Solution for BUG-13)
      addCourse({
        title: data.title,
      ...
      toast.error("Failed to publish course. Please check your connection.");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700 text-left pb-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {/* السعر والتخصص (Pricing & Category) */}
        <div className="space-y-8">
          ...
          {/* منطقة رفع الصورة (UX Visual Placeholder) */}
          <div className="bg-blue-50/50 p-8 rounded-[2.5rem] border-2 border-dashed border-blue-200 flex flex-col items-center justify-center gap-3 cursor-pointer hover:bg-blue-100/50 transition-all group">
            ...
            <p className="text-blue-600 font-bold text-sm">Upload Cover Image</p>
          </div>
        </div>
