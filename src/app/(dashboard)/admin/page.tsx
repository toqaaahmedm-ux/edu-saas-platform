...
export default function AdminDashboard() {
  ...
  // إحصائيات حقيقية من الملفات
  const totalStudents = USERS_DATA.filter(u => u.role === 'STUDENT').length;
  const totalTeachers = USERS_DATA.filter(u => u.role === 'TEACHER').length;
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 text-left">
      ...
      <div className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-slate-400 font-medium text-lg italic">
            Welcome back, Toqaa. Monitoring {USERS_DATA.length} active members across the platform.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* 1. Course Review Section (2/3 width) */}
            ...
            {COURSES.slice(0, 2).map((course) => (
              <div key={course.id} className="flex flex-col md:flex-row justify-between items-center p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white transition-all group">
                ...
                <div className="flex gap-3 mt-4 md:mt-0">
                  <button className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl font-bold hover:bg-emerald-600 hover:text-white transition-all">
                    <CheckCircle size={16} /> Approve
                  </button>
                  <button className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-600 hover:text-white transition-all">
        ...
        {/* 2. User Management Mini-List (1/3 width) */}
        <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
          ...
          <div className="space-y-4">
            {USERS_DATA.slice(0, 4).map((user) => (
              <div key={user.id} className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-xl transition-all">
