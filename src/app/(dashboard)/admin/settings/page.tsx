  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 text-left">
      ...
      <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm max-w-2xl">
        <h3 className="text-lg font-black text-slate-800 mb-6">Plan Usage</h3>
        {usageLoading ? (
          ...
          <div className="space-y-5">
            <UsageBar label="Users" current={usage.users?.current ?? 0} limit={usage.users?.limit ?? null} icon={Users} />
            <UsageBar label="Courses" current={usage.courses?.current ?? 0} limit={usage.courses?.limit ?? null} icon={BookOpen} />
            <UsageBar label="Storage (GB)" current={usage.storageGb?.current ?? 0} limit={usage.storageGb?.limit ?? null} icon={HardDrive} />
          </div>
        ) : (
          <p className="text-slate-400 text-sm italic">Usage data unavailable.</p>
        )}
      </div>

      <div className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm max-w-2xl">
        <h3 className="text-lg font-black text-slate-800 mb-6">Branding</h3>
        <form onSubmit={handleSave} className="space-y-6">
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2">
              <Building2 size={14} /> Organization Display Name
            </label>
            <input
              type="text"
              value={form.displayName}
              onChange={(e) => setForm({ ...form, displayName: e.target.value })}
              placeholder="e.g. Ain Shams Academy"
